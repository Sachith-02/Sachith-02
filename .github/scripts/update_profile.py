...
def build_focus_areas_section(config: Dict[str, Any]) -> str:
    """Render a clean professional focus table from profile.config.json."""
    areas = config.get("professional_focus_areas") or []
    if not areas:
        areas = [
            {
                "title": "Backend APIs",
                "summary": "Build maintainable service layers, REST APIs, and database-backed workflows.",
                "signal": "Java · Spring Boot · SQL",
            },
            {
                "title": "Automation",
                "summary": "Use GitHub Actions and Python scripts to remove repeated manual work.",
                "signal": "Actions · Python",
            },
        ]
    ...
    for item in areas:
        rows.append(
            "| "
            f"**{escape_md(item.get('title', 'Engineering'))}**"
            " | "
            f"{escape_md(item.get('summary', 'Configured in profile.config.json'))}"
            " | "
            f"`{escape_md(item.get('signal', 'Portfolio signal'))}`"
            " |"
        )

    open_to = config.get("open_to") or []
    open_to_block = ""
    if open_to:
        open_to_block = "\n\n**Open to:** " + " · ".join(f"{escape_md(item)}" for item in open_to)

    principles = config.get("profile_principles") or []
    principle_block = ""
    if principles:
        principle_block = "\n\n**Engineering principles:** " + " · ".join(f"`{escape_md(item)}`" for item in principles[:4])

    return f"""<!-- FOCUS_AREAS_START -->
## 🎯 Professional Focus
...
|---|---|---|
{chr(10).join(rows)}{open_to_block}{principle_block}

<!-- FOCUS_AREAS_END -->"""


def build_roadmap_section(config: Dict[str, Any]) -> str:
    """Render an upgrade roadmap so visitors see direction and maintenance intent."""
    items = config.get("portfolio_roadmap") or []
    if not items:
        items = [
            {"stage": "Now", "goal": "Improve portfolio quality", "deliverable": "README, CI, docs, and releases"},
            {"stage": "Next", "goal": "Add deeper case studies", "deliverable": "Architecture notes and API documentation"},
        ]
    ...
    for item in items:
        rows.append(
            "| "
            f"**{escape_md(item.get('stage', 'Next'))}**"
            " | "
            f"{escape_md(item.get('goal', 'Portfolio improvement'))}"
            " | "
            f"{escape_md(item.get('deliverable', 'Documented deliverable'))}"
            " |"
        )
    ...
    gates_block = ""
    if gates:
        gates_block = "\n\n**Quality gates:** " + " · ".join(f"`{escape_md(gate)}`" for gate in gates)
...
|---|---|---|
{chr(10).join(rows)}{gates_block}

<!-- ROADMAP_END -->"""

def build_actions_dashboard_section(config: Dict[str, Any]) -> str:
    """Render this profile repository's own automation workflows as a dashboard."""
    username = config["username"]
        ...
        workflows = [
            {
                ...
                "file": "update-profile.yml",
                "trigger": "Scheduled + manual",
                "purpose": "Regenerates the profile README",
            }
        ]

    rows = []
    for item in workflows:
        name = item.get("name") or item.get("file") or "Workflow"
        file_name = item.get("file") or "update-profile.yml"
        trigger = item.get("trigger") or "Manual"
        purpose = item.get("purpose") or "Profile automation"
        rows.append(
            "| "
            f"{workflow_badge(username, username, file_name, name)}"
            " | "
    ...
    body = "\n".join(rows)
    return f"""<!-- ACTIONS_DASHBOARD_START -->
...
def build_engineering_matrix_section(
    repos: List[Dict[str, Any]],
    language_totals: Dict[str, int],
    config: Dict[str, Any],
    scores: Dict[str, RepoScore],
) -> str:
    """Create a concise skill/evidence matrix from config and live repository data."""
    domains = config.get("engineering_domains") or []
    if not domains:
        domains = [
            {"domain": "Backend APIs", "level": 85, "evidence": "REST, services, databases"},
            {"domain": "Automation", "level": 80, "evidence": "GitHub Actions and generated README blocks"},
        ]

    rows = []
    for item in domains:
        domain = escape_md(item.get("domain", "Engineering"))
        level = float(item.get("level", 0) or 0)
        level = min(max(level, 0), 100)
        evidence = escape_md(item.get("evidence", "Configured in profile.config.json"))
        rows.append(f"| **{domain}** | `{progress_bar(level, width=12)}` **{level:.0f}%** | {evidence} |")

    best_repo = "Projects are still being scored"
    if repos:
        strongest = max(repos, key=lambda r: scores.get(r.get("name", ""), score_repo(r)).score)
        best_repo = f"[{escape_md(strongest.get('name'))}](https://github.com/{config['username']}/{strongest.get('name')})"

    top_languages = get_top_languages_text(language_totals, limit=5)
    svg_line = "<img src=\"assets/engineering-matrix.svg\" alt=\"Engineering matrix generated by GitHub Actions\" width=\"100%\"/>"

    return f"""<!-- ENGINEERING_MATRIX_START -->
## 🧬 Engineering Matrix
...
**Current strongest repository signal:** {best_repo}  
**Top language signal:** {escape_md(top_languages)}

<!-- ENGINEERING_MATRIX_END -->"""


def next_repo_action(repo: Dict[str, Any]) -> str:
    """Suggest one profile-improving action for a repository."""
    if not repo.get("description"):
        return "Add a clear project description"
    if not repo.get("topics"):
        return "Add GitHub topics"
    if not repo.get("license"):
        return "Add a license if appropriate"
    if int(repo.get("open_issues_count") or 0) > 0:
        return "Triage open issues"
    return "Keep releases and docs updated"


def build_repo_health_section(repos: List[Dict[str, Any]], config: Dict[str, Any], scores: Dict[str, RepoScore]) -> str:
    """Show a repository quality board for portfolio improvement."""
    username = config["username"]
    ranked = sorted(repos, key=lambda r: scores.get(r.get("name", ""), score_repo(r)).score, reverse=True)
    rows = []
    for repo in ranked[:8]:
        name = repo.get("name", "")
        language = repo.get("language") or "—"
        topics_count = len(repo.get("topics") or [])
        desc_status = "✅" if repo.get("description") else "⚠️"
        score = scores.get(name, score_repo(repo)).score
        action = next_repo_action(repo)
        rows.append(
            "| "
            f"[{escape_md(name)}](https://github.com/{username}/{name})"
            " | "
            f"{escape_md(language)}"
            " | "
            f"{desc_status} description · `{topics_count}` topics"
            " | "
            f"**{score:.1f}**"
            " | "
            f"{escape_md(action)}"
    ...
    if not rows:
        rows.append("| No repositories found yet | — | — | 0 | Push public projects first |")

    return f"""<!-- REPO_HEALTH_START -->
...
def build_automation_architecture_section(config: Dict[str, Any]) -> str:
    """Describe the profile automation architecture using Mermaid and a generated SVG."""
    workflow_count = len(config.get("profile_workflows") or [])
    section_count = len(config.get("profile_sections") or [])
    return f"""<!-- AUTOMATION_ARCHITECTURE_START -->
...
|---|---|
| **{workflow_count} profile workflows** | Split testing, generation, validation, maintenance, and release packaging |
| **{section_count} README modules** | Keeps the profile structured, dynamic, and easy to extend |
...
def main() -> int:
    ...
    content = replace_block(content, "FOCUS_AREAS", build_focus_areas_section(config))
    content = replace_block(content, "ACTIONS_DASHBOARD", build_actions_dashboard_section(config))
    content = replace_block(content, "ENGINEERING_MATRIX", build_engineering_matrix_section(visible_repos, language_totals, config, scores))
    ...
    content = replace_block(content, "REPO_HEALTH", build_repo_health_section(visible_repos, config, scores))
    content = replace_block(content, "ACTIVITY", build_activity_section(events, int(config.get("max_recent_activity", 6))))
    content = replace_block(content, "ROADMAP", build_roadmap_section(config))
    content = replace_block(content, "AUTOMATION_ARCHITECTURE", build_automation_architecture_section(config))
